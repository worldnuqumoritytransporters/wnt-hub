...
This is a node for WNT network that serves as [relay](../../../wnt-relay), plus facilitates exchange of end-to-end encrypted messages among devices connected to Wnt network.  The hub does not hold any private keys and cannot send payments itself.
...
Users set their hub address in their wallet settings.  The default hub is wss:/hub.worldnuqumoritytransporters.com but users can change it to your hub address.
...
You need a private key generated by Wnt Team for you, key_id and team_id to communicate with APNS servers on behalf of Wnt app bundle, email us for ones: support@wnt.main.jp, then configure your hub (conf.js or conf.json) to enable notifications.
