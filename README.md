# Hub for worldnuqumoritytransporters network

This is a node for worldnuqumoritytransporters network that serves as [relay](../../../wnt-relay), plus facilitates exchange of end-to-end encrypted messages among devices connected to Byteball network.  The hub does not hold any private keys and cannot send payments itself.
...
* Plain text chat with bots that offer a service and can receive or send payments.  [Faucet](../../../wnt-faucet) is an example of such bot.
...
npm install
```
Set up a proxy, such as nginx, to forward all websocket connections on a specific path to your daemon running the hub code.  See example configuration for nginx in [wntcore](../../../wntcore) documentation.
...
If you want to change any defaults, refer to the documentation of [wntcore](../../../wntcore), the core Byteball library `require()`'d from here.
